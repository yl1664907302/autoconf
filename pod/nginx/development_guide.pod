...
C<ngx_explicit_memzero()> — Does the same as
C<ngx_memzero()>, but this call is never removed by the
...
C<ngx_vslprintf(buf, last, fmt, args)>



=item *

C<ngx_vsnprintf(buf, max, fmt, args)>
...
expression C<re>, the string to match C<input>,
...
The C<size>
should be at least C<NGX_MIN_POOL_SIZE>
and a multiple of C<NGX_POOL_ALIGNMENT>.
...
in the past truncated to C<ngx_msec_t> type.
Its current value can be obtained from the C<ngx_current_msec>
variable.
...
The C<ngx_post_event(ev, q)> macro posts the event
...
A new C<ngx_connection_t> object is created at this stage
...
C<r-E<gt>out> of type C<ngx_buf_t>.
...
the C<ngx_http_top_request_body_filter> variable.
It's assumed that every body handler calls the next handler in the chain until
the final handler C<ngx_http_request_body_save_filter(r, cl)>
is called.
This handler collects the buffers in
C<r-E<gt>request_body-E<gt>bufs>
and writes them to a file if necessary.
The last request body buffer has nonzero C<last_buf> flag.
...
C<r-E<gt>request_body-E<gt>filter_need_buffering> to
C<1> when called for the first time.
...
The
L<nginx-dev-examples|http://hg.nginx.org/nginx-dev-examples>
...
C<ngx_> or more specific prefix such as
C<ngx_http_> and C<ngx_mail_>
...
The C<ngx_config.h> and C<ngx_core.h> files
are always included first, followed by one of
C<ngx_http.h>, C<ngx_stream.h>,
or C<ngx_mail.h>.
...
“C<E<sol>E<sol>>” comments are not used
...
Macro names start from C<ngx_> or C<NGX_>
...
Type names end with the “C<_t>” suffix.
...
Structure types are defined using C<typedef>.
...
Enumerations have types ending with “C<_e>”:
...
There is a bunch of commonly used typeE<sol>name combinations:
...
The C<ngx_inline> macro should be used instead of
C<inline>:
...
Binary operators except “C<.>” and “C<−E<gt>>”
...
=head2 Conditionals and Loops



The “C<if>” keyword is separated from the condition by
one space.
...
by “C<else if> E<sol> C<else>”.
Usually, there is an empty line before the
“C<else if> E<sol> C<else>” part:
...
Similar formatting rules are applied to “C<do>”
and “C<while>” loops:
...
The “C<switch>” keyword is separated from the condition by
one space.
Opening brace is located on the same line.
Closing brace is located on a dedicated line.
The “C<case>” keywords are lined up with
“C<switch>”:
...
Most “C<for>” loops are formatted like this:
...
If some part of the “C<for>” statement is omitted,
this is indicated by the “C<E<sol>* void *E<sol>>” comment:
...
“C<E<sol>* void *E<sol>>” comment which may be put on the same line:
...
=head1 Debugging memory issues



To debug memory issues such as buffer overruns or use-after-free errors, you
can use the L<AddressSanitizer|https://en.wikipedia.org/wiki/AddressSanitizer> (ASan) supported by some modern compilers.
To enable ASan with C<gcc> and C<clang>,
use the C<-fsanitize=address> compiler and linker option.
When building nginx, this can be done by adding the option to
C<--with-cc-opt> and C<--with-ld-opt>
parameters of the C<configure> script.
...
C<NGX_DEBUG_PALLOC> macro to C<1>.
...
such as L<Perl|ngx_http_perl_module>
or L<njs|index>?
...
such as C<strlen> or C<strstr>.
Instead, nginx counterparts
that accept either C<ngx_str_t> should be used
or pointer to data and a length.
However, there is a case when C<ngx_str_t> holds
...
=head2 Manual Memory Management



Instead of dealing with mallocE<sol>free approach which is error prone,
...
the proper way is to schedule a timer in the C<init_process>
...
=head2 HTTP Requests to External Services
...
L<proxying|ngx_http_proxy_module>
requests, but may contain other nginx directives.
An example of such approach is the
L<ngx_http_auth_request_module>
directive implemented in
L<ngx_http_auth_request module|http://hg.nginx.org/nginx/file/tip/src/http/modules/ngx_http_auth_request_module.c>.
...
available in nginx.
For example,
L<OCSP module|http://hg.nginx.org/nginx/file/tip/src/event/ngx_event_openssl_stapling.c>
